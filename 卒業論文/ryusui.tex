...
\section{リアルタイム照明制御システムの実装} \label{sec:system}
...
これにより、演奏者は音楽表現そのものに集中でき、照明演出が演奏の妨げとなることを防いでいる。
...
本システムは以下のハードウェアで構成される(\tabref{tab:hardwea})。
...
コンデンサマイク(音声入力)  \\
スマートフォン(ZIG-SIM用) \\
USB-DMXインタフェース(\figref{fig:usbdmx})  \\
DMX対応照明機器4台(\figref{fig:light})  \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{../fig/usb-dmx-interface.png}
...
ライブラリ名 & 役割  \\
\hline
sounddevice\cite{SounddevicePlayRecord} & 音声ストリーム取得 \\
...
\subsection{音響特徴量抽出} \label{sec:sound}
...
本システムでは、演奏者の弓の上下運動(上げ弓・下げ弓)(\figref{fig:up}\cite{GongmatusuguhenoDaoGongdakenoLianXiFa1Ri5FenOnrainbaiorinretusunnaraharukaXianSheng}・\figref{fig:down}\cite{GongmatusuguhenoDaoGongdakenoLianXiFa1Ri5FenOnrainbaiorinretusunnaraharukaXianSheng})を取得するために、スマートフォンをZIG-SIMセンサとして利用している。
...
\begin{enumerate}
	\item[(1)]上げ弓：DMX値を120に設定（照明上昇）(\figref{fig:lightupdown}参照)
	\item[(2)]下げ弓：DMX値を0に設定（照明下降）(\figref{fig:lightupdown}参照)
\end{enumerate}
...
\figref{fig:dataflow}に、照明予測モデル構築のためのデータ生成フローを示す。
...
\subsection{データベースの構築} \label{sec:database}
...
2つ目のライブ映像データは、ヴァイオリンソロによるクラシック楽曲「ソナタ第三番\cite{VuaiorinsonataDi3FanBetovuenBaiKeShiDian}」を演奏した映像(\figref{fig:sonata})である\cite{aiolinofficialLIVEHikaritoViolinSolo2019}。
...
\tabref{tab:soundcsv}に、各ライブ映像から抽出された音響特徴量のCSVデータの一部を示す。
...
照明特徴量の抽出において最も重要な前処理は、YOLOv8\cite{YOLOv8StateoftheArtComputer} を用いた人物領域の除外である。
...
\centering
\small
\caption{ソナタ第三番のRGB値のCSVデータ(一部抜粋)}
\label{tab:sonatargbcsv}
\begin{tabular}{c|ccc}
\hline
\hline
Time [s] & R & G & B \\
...
これらの値をフレーム単位でバッファに蓄積し、1秒ごとに平均を取ることで、1秒単位のRGB値の照明特徴量としてCSVに書き出した(\tabref{tab:senbonzakurargbcsv},\tabref{tab:sonatargbcsv},\tabref{tab:splashrgbcsv})。
...
また、RGB値と同様に、1秒ごとに平均を取ることで、1秒単位のBrightness値としてCSVに書き出した(\tabref{tab:senbonzakurabrightcsv},\tabref{tab:sonatabrightcsv},\tabref{tab:splashbrightcsv})。

\begin{table}[H]
\centering
\small
\caption{千本桜のBrightnessのCSVデータ(一部抜粋)}
\label{tab:senbonzakurabrightcsv}
\begin{tabular}{c|c}
\hline
\hline
Time [s] & Brightness  \\
...
\centering
\small
\caption{ソナタ第三番のBrightnessのCSVデータ(一部抜粋)}
\label{tab:sonatabrightcsv}
\begin{tabular}{c|c}
\hline
\hline
Time [s] & Brightness  \\
...
前後5秒の局所FFT を行うことで点滅に対応する周波数成分を推定し、csvに書き出した(\tabref{tab:senbonzakurablinkcsv},\tabref{tab:sonatablinkcsv},\tabref{tab:splashblinkcsv})。

\begin{table}[H]
\centering
\small
\caption{千本桜の点滅速度のCSVデータ(一部抜粋)}
\label{tab:senbonzakurablinkcsv}
\begin{tabular}{c|c}
\hline
\hline
...
\centering
\small
\caption{ソナタ第三番の点滅速度のCSVデータ(一部抜粋)}
\label{tab:sonatablinkcsv}
\begin{tabular}{c|c}
\hline
\hline
Time [s] & Blinking Speed  \\
...
\subsection{照明予測モデル(仮)の学習} \label{sec:modelthink}

\ref{sec:database}節で収集した学習用CSVデータを用いて、照明予測モデルの学習を行った。

このモデルは、ライブ映像由来の実測データのみを用いて学習された点に特徴がある。
一方で、3種類の映像のみでは演奏種類やデータ量としては少ないため、本研究ではこのモデルを「照明予測モデル（仮）」と位置付けている。

\subsubsection{学習データの概要}

照明予測モデル(仮)の学習には、\ref{sec:database}節にて生成した音響特徴量と照明特徴量のCSVのデータを1つのデータセットとして統合し、使用した(\tabref{tab:csv})。

\subsubsection{入力変数（音響特徴量）}

照明予測モデル(仮)への入力として使用した音響特徴量のデータセットは\ref{sec:soundmodel}節で述べた通りである。

\subsubsection{出力変数（照明特徴量）}

照明予測モデル(仮)の出力として使用した照明特徴量のデータセットは\ref{sec:lightmodel}節で述べた通りである。

\subsubsection{モデルの構造と学習方法}

照明予測モデル（仮）には、回帰モデルとして Random Forest Regressor\cite{RandomForestRegressor} を採用した。
...
\subsection{MIDIデータの生成と特徴量抽出} \label{sec:midi}

本節では、\ref{sec:modelthink}節で構築した照明予測モデル（仮）を用いて、MIDI形式のヴァイオリン楽曲データから音響特徴量および照明特徴量を生成する手法について述べる。
...
\centering
\small
\caption{各映像の音響特徴量と照明特徴量の統合データ(一部抜粋)}
\label{tab:csv}
\begin{tabular}{c|cccccccccc}
\hline
\multicolumn{11}{c}{千本桜} \\
...
\hline
1.0 & 8894 & 14.5 & 0.001 & -948 & 0.67 & 90.1 & 45.7 & 62.1 & 59.8 & 0.40 \\
2.0 & 6722 & 17.9 & 0.005 & -661 & 0.76 & 93.9 & 60.2 & 74.1 & 69.3 & 0.33 \\
3.0 & 5257 & 23.2 & 0.033 & -385 & 0.57 & 89.8 & 60.7 & 70.0 & 87.0 & 0.29 \\
4.0 & 4459 & 21.7 & 0.023 & -398 & 0.75 & 84.2 & 42.5 & 53.6 & 53.5 & 0.25 \\
5.0 & 3987 & 19.5 & 0.028 & -358 & 0.28 & 84.8 & 32.2 & 45.8 & 47.7 & 0.44 \\
6.0 & 3678 & 18.3 & 0.050 & -347 & 0.37 & 130.6 & 16.2 & 31.1 & 53.3 & 0.40 \\
7.0 & 3521 & 17.8 & 0.039 & -410 & 0.30 & 125.8 & 126.4 & 136.2 & 56.7 & 0.40 \\
8.0 & 3400 & 16.7 & 0.008 & -471 & 0.43 & 124.7 & 35.9 & 38.0 & 63.3 & 0.20 \\
\hline
\multicolumn{11}{c}{ソナタ第三番} \\
\hline
\hline
Time [s] & Centroid & Contrast & RMS &　MFCC\_1 & Chroma\_1 & R & G & B
 & Brightness & Blink Speed  \\
\hline
1.0 & 1947 & 9.04 & 0.001 & -1068 & 0.04 & 50.8 & 45.1 & 68.7 & 50.2 & 0.40 \\
...
Time [s] & Centroid & Contrast & RMS &　MFCC\_1 & Chroma\_1 & R & G & B
 & Brightness & Blink Speed  \\
\hline
1.0 & 1002 & 0.00 & 0.000 & -1131 & 0.08 & 91.6 & 70.0 & 76.5 & 77.5 & 0.40 \\
2.0 & 3582 & 8.98 & 0.008 & -1055 & 0.27 & 54.1 & 36.1 & 50.5 & 35.3 & 0.33 \\
3.0 & 1603 & 2.24 & 0.001 & -1109 & 0.16 & 17.8 & 11.4 & 23.9 & 19.2 & 0.29 \\
...
抽出する特徴量の種類および構成は、\ref{sec:modelthink}節で構築した照明予測モデル（仮）の学習時に用いたデータセットと完全に一致させている。
...
1秒単位に集約された音響特徴量は、\ref{sec:modelthink}節で構築した 照明予測モデル（仮） に入力される。
本モデルは、ライブ映像データから学習した「音響特徴量と照明特徴量の対応関係」を保持しており、MIDI由来の音響特徴量に対しても照明特徴量を推定することが可能である。
推定される照明特徴量はライブ映像データと同様、RGB値、Brightness、点滅速度の3種類である。
...
\subsection{MIDIデータを含めた照明予測モデルの学習} \label{sec:midimodel}

\ref{sec:midi}節で生成したMIDIデータ由来の特徴量データを加え、照明予測モデルを再学習する手法について述べる。
本節で構築されるモデルは、ライブ映像データに加えてMIDIデータも含めた拡張データセットを用いて学習されたモデルであり、以降「照明予測モデル(本)」と呼ぶ。

\subsubsection{学習データセットの構成}

本研究では、\ref{sec:midi}節で生成したMIDIデータ由来の特徴量データを、\ref{sec:database}節で構築したライブ映像データ由来の特徴量データに追加し、拡張された学習データセットを構成した。

MIDIデータについては、実際の照明映像を伴わないため、\ref{sec:modelthink}節で学習した照明予測モデル（仮）を用いて照明パラメータを推定し、擬似的な教師データとして扱っている。

\subsubsection{特徴量構成と前処理}

入力特徴量および出力変数の構成は、\ref{sec:modelthink}節で使用したものと同一であるため、本節では詳細な説明を省略する。
...
本節では、\ref{sec:midimodel}節で構築した MIDIデータを含めた照明予測モデル(本)の性能評価について述べる。
評価には、機械学習における回帰問題で一般的に用いられる決定係数（R²スコア）を採用した。

\subsubsection{評価指標：決定係数 (R²スコア)} \label{sec:r2}

本研究では、照明予測モデル(本)の性能を定量的に評価するため、決定係数 R²（Coefficient of Determination） (\figref{fig:r2-formula})を用いた。
...
最終照明予測モデル(本)に対する評価の結果、R²スコアは 0.85 を示した(\figref{fig:r2-result})。
...
\ref{sec:r2}節では、R²スコアを用いた定量的評価によりモデル全体の性能を示したが、本節ではさらに踏み込み、「どの音響的要素が、どの照明要素に強く影響しているか」を明らかにすることを目的とする。
...
解析には、\ref{sec:database}節および\ref{sec:midi}節で生成されたCSVデータを使用した。
...
\figref{fig:rlightstyle}は、照明色の R 成分（Average R）を目的変数として学習した Random Forest 回帰モデルにおける、音響特徴量の重要度上位5項目を円グラフで示したものである。
...
\figref{fig:glightstyle}は、照明色の G 成分（Average G）を目的変数として学習した Random Forest 回帰モデルにおける、音響特徴量の重要度上位5項目を示した結果である。
...
\figref{fig:blightstyle}は、照明色の B 成分（Average B）を目的変数として学習した Random Forest 回帰モデルにおける、音響特徴量の重要度上位5項目を示した結果である。
...
\figref{fig:brightnessstyle}は、照明の明るさ（Background Brightness）を目的変数として学習した Random Forest 回帰モデルにおける、音響特徴量の重要度上位5項目を示した結果である。
...
\figref{fig:blinkstyle}は、照明の点滅速度（Blinking Speed）を目的変数として学習した Random Forest 回帰モデルにおける、音響特徴量の重要度上位5項目を示した結果である。
...
\figref{fig:gurenge-intro}～\figref{fig:gurenge-chorus2}に、実演中の照明変化の様子を示す。
...
演奏開始直後のイントロ部分（\figref{fig:gurenge-intro}）では、演奏音のRMS値が全体を通して低い水準で推移しており、音量変動も小さい区間である。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{../fig/gurenge-intro.png}
	\caption{『紅蓮華』実演中の照明変化の様子（イントロ部分）}
	\label{fig:gurenge-intro}
\end{figure}
...
演奏が進行し、中盤部分（\figref{fig:gurenge-Amelody-Bmelody-Cmelody}）に入ると、RMS値の平均および最大値が徐々に増加し、それに伴ってBrightnessも段階的に上昇していることが確認できる。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{../fig/gurenge-Amelody-Bmelody-Cmelody.png}
	\caption{『紅蓮華』実演中の照明変化の様子（Aメロ～Cメロ）}
	\label{fig:gurenge-Amelody-Bmelody-Cmelody}
\end{figure}
...
さらに、楽曲がサビ部分に近づくにつれて（\figref{fig:gurenge-chorus1},\figref{fig:gurenge-chorus2}）、RMS値は高い値を維持するとともに、短時間内での変動幅も大きくなっている。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{../fig/gurenge-chorus1.png}
	\caption{『紅蓮華』実演中の照明変化の様子（サビ部分）}
	\label{fig:gurenge-chorus1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{../fig/gurenge-chorus2.png}
	\caption{『紅蓮華』実演中の照明変化の様子（サビ部分）}
	\label{fig:gurenge-chorus2}
\end{figure}
...
これは、音量が小さい区間では視覚的刺激を抑制するという意図と一致しており、楽曲の導入部として適切な雰囲気を形成できている。
...
第4に、演奏者や楽器への照射は強調されているものの、空間全体としての照明バランスや観客視点での視認性については、さらなる検討の余地がある。
